"""Spawn an eta robot in new Gazebo, also launch the robot_state_publisher."""
...
PKG_eta_DESCRIPTION = get_package_share_directory('eta_description')
...
def get_robot_description() -> str:
    ...
    robot_description_file_path = os.path.join(PKG_eta_GZ, 'urdf', 'eta_gz.urdf.xacro')
    ...
    # Passing absolute path to the robot description due to Gazebo issues finding eta_description pkg path.
    robot_desc = robot_desc.replace(
        'package://eta_description/', f'file://{PKG_eta_DESCRIPTION}/'
    )
    return robot_desc
...
def generate_launch_description():
    ...
    entity_argument = DeclareLaunchArgument(
        'entity', default_value='eta', description='Name of the robot.'
    )
    x_argument = DeclareLaunchArgument(
        'initial_pose_x',
        default_value='0.0',
        description='Initial x pose of eta in the simulation.',
    )
    y_argument = DeclareLaunchArgument(
        'initial_pose_y',
        default_value='0.0',
        description='Initial y pose of eta in the simulation',
    )
    z_argument = DeclareLaunchArgument(
        'initial_pose_z',
        default_value='0.1',
        description='Initial z pose of eta in the simulation.',
    )
    yaw_argument = DeclareLaunchArgument(
        'initial_pose_yaw',
        default_value='0.0',
        description='Initial yaw pose of eta in the simulation.',
    )
