...
from eta_gz.launch_tools.substitutions import TextJoin


def generate_launch_description():
    pkg_eta_gz = get_package_share_directory('eta_gz')
    ...
    robots_arg = DeclareLaunchArgument(
        'robots', default_value="eta={x: 0., y: 0., z: 0.1, yaw: 0.};",
    ...
    params_file_arg = DeclareLaunchArgument(
        'params_file',
        default_value=PathJoinSubstitution([pkg_eta_gz, 'config', 'nav2_params.yaml']),
    ...
    gui_config_path = PathJoinSubstitution([pkg_eta_gz, 'config_gui', gui_config])
    nav2_flag = LaunchConfiguration('nav2')
    params_file = LaunchConfiguration('params_file')
    autostart = LaunchConfiguration('autostart')

    # Obtains world path.
    world_path = PathJoinSubstitution([pkg_eta_gz, 'worlds', world_name])
    log_world_path = LogInfo(msg=TextJoin(substitutions=["World path: ", world_path]))
    # Obtains the map path.
    map_path = PathJoinSubstitution([pkg_eta_gz, 'maps', map_name, TextJoin(substitutions=[map_name ,'.yaml'])])
    ...
    if (robots_list == {}):
        ...
        robots_list = {"eta": {"x": 0., "y": 0., "z": 0.1, "yaw": 0.}}
    ...
    for robot_name in robots_list:
        ...
        robots_group = GroupAction(
            ...
            actions=[
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(pkg_eta_gz, 'launch', 'include', 'spawn_robot.launch.py')
                    ),
                    launch_arguments={
                        'entity': robot_name,
                ...
                Node(
                    ...
                    package='rviz2',
                    executable='rviz2',
                    arguments=['-d', os.path.join(pkg_eta_gz, 'rviz', 'eta_gz_nav2.rviz')],
                ...
                Node(
                    ...
                    package='rviz2',
                    executable='rviz2',
                    arguments=['-d', os.path.join(pkg_eta_gz, 'rviz', 'eta_gz.rviz')],
                    parameters=[{'use_sim_time': True}],
                    remappings=[
                        ('/tf', 'tf'),
                        ('/tf_static', 'tf_static'),
                ...
                IncludeLaunchDescription(
                    PythonLaunchDescriptionSource(
                        os.path.join(pkg_eta_gz, 'launch', 'include', 'gz_ros_bridge.launch.py')
                    ),
                    launch_arguments={
                        'entity': robot_name,
                    }.items(),
